    ...
    public class TravelRequestsController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            using (BTCEntities db = new BTCEntities())
            {
                ...
                {
                    return View(await db.TravelRequests.Include(t => t.City).Include(t => t.City1).Include(t => t.Currency).Include(t => t.Users1).Include(t => t.Users).Where(a => a.IsSubmitted == true && a.IsDeleted == false).ToListAsync());
                }
                else
                {
                    return View(await db.TravelRequests.Include(t => t.City).Include(t => t.City1).Include(t => t.Currency).Include(t => t.Users1).Include(t => t.Users).Where(a => a.CreatedBy == dbuser.ID && a.IsDeleted == false).ToListAsync());
                }
            }
        }
